...
+ `if_else` is a wrapper around `base::ifelse()` that can take mappers. 
...
Mappers are function built like one sided formulas. In other words, `~ .x + 2` is the equivalent to `function(x) return(x + 2)`. The first argument in a mapper need to be `.x`.
