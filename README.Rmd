...
{attempt} is designed to handle the cases when something / someone attempts to do something it shouldn't. For example, if you : 
+ attempt to run a `log("a")` 
+ attempt to connect to a web API without an internet connexion running

{attempt} provides several condition handlers, from try catch to message printing. 
...
`silently` does the same job as `try`, except it doesn't return the value if the condition is verified.
